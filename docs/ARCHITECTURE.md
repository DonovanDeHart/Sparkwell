...
- `embeddings(spark_id, model, dimensions, vector BLOB, content_hash, generated_at, profile_version)`: a vector cache only. Vectors are loaded only for the current model and profile recipe (`profile_version`, migration 0002) with a matching content hash; when indexing starts, rows from any other model or recipe are deleted and the library is re-indexed in the background while standard search stays available. Spark content is never touched.
...
4. **Retrieval profiles** (`search::profile`, recipe `profile-v3`). The profile is for *finding* a Spark; the stored body is for *using* it, and is never changed. Each Spark is embedded as several views: a compact, labelled profile (`Title:`, `Summary:`, `Purpose:` — its opening instructions, `Needs:` — the input it asks for, `Tags:`, `Covers:` — topics sampled across its sections) and up to four passages spread across the body, each prefixed with the Spark's identity. A long, detailed Spark is therefore compared on what it is for, not diluted by its length, and a short one is not favoured just for being short. Profiles are derived deterministically; no model rewrites anything.
5. **Semantic score** (`search::vectors`). Queries carry the Qwen3 Embedding task instruction in the model card's format — `Instruct: Given a user's goal, retrieve the reusable AI prompt whose intended purpose best helps accomplish it, matching intention and desired outcome rather than exact wording\nQuery: {goal}` — and documents are embedded without one (asymmetric, as the model card recommends). Similarity to a Spark is a soft maximum over its views (0.75 × best + 0.25 × second, profile weighted 1.05). A fixed pool of 40 generic, unrelated goals embedded the same way provides two corrections: a Spark's *hub level* (its mean similarity to the pool) is subtracted, so Sparks written in generic "AI assistant" language don't match everything, and the result is scaled by ten times the typical spread of similarities (≈0.53 on the acceptance library). At least 5 indexed Sparks are needed; below that, standard retrieval is used.
6. **Fusion:** 0.8 × semantic + 0.2 × IDF-weighted lexical (Sparks not yet indexed use 0.85 × lexical). An exact title match is floored at 0.95. Favorite, usage and recency only break ties. All values live in one `search::CALIBRATION` for the canonical model; `tests/semantic_tune.rs` sweeps them against recorded vectors.
7. **Confidence.** "No strong match" is always preferred to a wrong answer:
   - With semantic evidence, a **Best Match** needs a fused score ≥ 0.24 and a lead of at least 0.18 over the runner-up, unless the two leaders state essentially the same purpose (profile similarity ≥ 0.9). The margin is the zero-wrong choice: among all tuning and validation goals the closest confidently wrong candidate leads by 0.16, so anything at or below that lets a wrong Spark through (at 0.12, 9/12 acceptance goals become Best Matches but one paraphrase is confidently wrong). Candidates below 0.10 are not shown.
...
**Evaluation.** `src-tauri/tests/support` holds the 12 goals from the physical acceptance test (each with its expected Spark and acceptable alternatives), 36 paraphrases (tuning), 23 validation goals, 20 final test goals never used for any choice, 16 unrelated goals, and 10 goals for five user-added Sparks pasted the way people capture prompts from ChatGPT, Claude or Gemini (`tests/fixtures/user_added_sparks.json`, with CRLF, tabs, emoji and surrounding whitespace). Results are graded Correct / Acceptable / Missed / Confidently wrong. `semantic_regression.rs` replays recorded `qwen3-embedding:8b-q8_0` vectors in CI; `semantic_live.rs` (opt-in) runs the same suites against a live Ollama, reports timings, and can re-record the fixture; `user_added_sparks.rs` checks those Sparks round-trip byte for byte.
...
- **One canonical embedding model:** `qwen3-embedding:8b-q8_0` (Qwen3 Embedding 8B, Q8_0, 4096 dimensions, ~9 GB of GPU memory while loaded). The whole pipeline (query instruction, profile recipe, `search::CALIBRATION`) is calibrated and tested for it, so Sparkwell never substitutes another installed embedding model; without it, standard search is used and the UI says "semantic model not installed" / "Local semantic search is not ready" with the `ollama pull` command. Sparkwell never downloads a model itself. `SPARKWELL_EMBED_MODEL` exists for development only. Cloud models are never used, and embedding models are never picked as chat models.
- **Query embeddings** (measured on an RTX 5090: ~2.3 s to load the model, then ~30 ms per query, one ~4.6 s hiccup right after a load) get 8 s while the model is known to be loaded and 30 s when it may be cold; showing the panel pre-warms it (kept loaded for 30 min, so it isn't reloaded between searches). Background indexing sends batches of 16 texts (about 0.3 s each). After 1.2 s the pending state reads "Waking up local intelligence…" and the rest of the panel stays usable. A timeout or failure falls back to standard search **with a label saying why**, and schedules a re-probe.
